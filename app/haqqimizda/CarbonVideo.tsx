...
export default function CarbonVideo({
  ...
  useEffect(() => {
    ...
    const capturePoster = () => {
      ...
      try {
        ...
        // Poster generation temporarily seeks into the video.
        // Always restore the real playback position to 0:00.
        try {
          video.currentTime = 0;
        } catch {
          // Ignore browsers that temporarily reject seeking.
        }
  ...
  const playVideo = async () => {
    ...
    // The first intentional play always starts from the beginning.
    // This prevents the generated-poster seek position from leaking
    // into actual playback.
    if (!started) {
      try {
        video.currentTime = 0;
      } catch {
