...
function ReservationPanel({
  ...
  const phoneHref = "tel:+994554840006";
  ...
  return (
    ...
    >
      ...
      <div className="carbon-date-grid">
        <label className="carbon-book-field">
          <span>
      ...
      <div className="carbon-duration-strip">
        <div>
          <Clock3 size={14} />
          <span>Müddət</span>
        </div>
      ...
      <div className="carbon-reserve-select-wrap">
        <button
          type="button"
          className={`carbon-reserve-select ${
            pickupOpen ? "open" : ""
          }`}
          onClick={() => setPickupOpen((value) => !value)}
        >
          <span className="carbon-reserve-select-icon">
            <selectedPickup.icon size={17} />
          </span>

          <span>
            <small>TƏHVİL ÜSULU</small>
            <strong>{selectedPickup.title}</strong>
          </span>

          <ChevronDown
            size={15}
            className={pickupOpen ? "rotate" : ""}
          />
        </button>
        ...
        <AnimatePresence>
          {pickupOpen && (
            <motion.div
              className="carbon-reserve-dropdown"
      ...
      <div className="carbon-driver-row">
        <div>
          <Users size={15} />

          <span>
            <small>SÜRÜCÜ SAYI</small>
            <strong>
              {drivers === 1 ? "1 sürücü" : "2 sürücü"}
            </strong>
          </span>
        </div>

        <div className="carbon-stepper">
          <button
            type="button"
            onClick={() =>
              setDrivers((value) => Math.max(1, value - 1))
            }
            aria-label="Sürücü sayını azalt"
          >
            <Minus size={12} />
          </button>
          ...
          <button
            type="button"
            onClick={() =>
              setDrivers((value) => Math.min(2, value + 1))
            }
            aria-label="Sürücü sayını artır"
          >
            <Plus size={12} />
          </button>
        </div>
